...
B30_SHIELD_DEFAULTS = {
    "B30": {"gamma": 80.0, "power": 8},
    "B30-FT-E-M": {"gamma": 3.0, "power": 3},
}

for _base_name in B30_SHIELD_BASE_NAMES:
    paths[f"{_base_name}-Shield"] = paths[_base_name]
...
def is_shield_model_name(name: str) -> bool:
    return name.endswith("-Shield")


def strip_shield_suffix(name: str) -> str:
    return name[:-len("-Shield")] if is_shield_model_name(name) else name


def shield_defaults_for(base_name: str) -> dict:
    return B30_SHIELD_DEFAULTS.get(base_name, {"gamma": 3.0, "power": 3})


def load_models(model_names, shield_pgd_mode: str = "white-box"):
    models = []
    attack_models = []
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    for name in model_names:
        is_shield = is_shield_model_name(name)
        base_name = strip_shield_suffix(name)

        if is_shield and base_name not in B30_SHIELD_BASE_NAMES:
            raise ValueError(
                f"Shield is only available for B30 models. Got {name!r}."
            )

        model_path = paths[name] if name in paths else paths[base_name]

        if base_name in config_paths:
            cfg_path = config_paths[base_name]
        ...
        elif base_name.startswith("ProtoVAE-FT-"):
        ...
        if is_shield:
            cfg = shield_defaults_for(base_name)
            ...
            attack_model = base_model if shield_pgd_mode == "grey-box" else model
            print(
    ...
    return models, model_names, attack_models
...
def main():
    ...
    # Load models
    models, model_names, attack_models = load_models(
        args.models,
        shield_pgd_mode=args.shield_pgd_mode,
    )

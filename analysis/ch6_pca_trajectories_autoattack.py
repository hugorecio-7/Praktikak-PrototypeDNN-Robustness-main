...
def autoattack_reference_points(
    ...
    """
    Run AutoAttack on the selected samples and return final examples plus z.

    AutoAttack does not expose a step-by-step trajectory, so this script plots
    its final latent code as a stronger reference point for each sample.
    """
...
def plot_trajectory_panel(
    ...
    for i in range(n_samples):
        ...
        # AutoAttack reference point for the same sample
        ax.scatter(
...
def plot_trajectories(
    ...
    auto_a_2d  = pca.transform(z_auto_a - mean_a)
    auto_b_2d  = pca.transform(z_auto_b - mean_b)
    ...
    plot_trajectory_panel(
        axes[0], traj_a_2d, auto_a_2d, proto_a_2d, proto_lbl_a, y,
        title=f"{label_a}  (ε={eps}, {iters} steps)",
    )
    plot_trajectory_panel(
        axes[1], traj_b_2d, auto_b_2d, proto_b_2d, proto_lbl_b, y,
    ...
    fig.suptitle(
        ...
        f"PCA fit on joint clean codes (mean-centred per model); AutoAttack={auto_version}",
        fontsize=11, y=1.02,
    )
    plt.tight_layout()
...
def main() -> None:
    ...
    parser.add_argument("--autoattack_version", type=str, default="standard",
                        help="AutoAttack version used for the reference point.")
    ...
    # ------------------------------------------------------------------
    # 6. Run AutoAttack final reference points for the same samples
    # ------------------------------------------------------------------
    print(
        f"Running AutoAttack ({args.autoattack_version}, eps={args.eps}) "
        f"on {args.model_a} ..."
    )
    _, z_auto_a = autoattack_reference_points(
        model=model_a,
        x=x_sub,
        y=y_sub,
        eps=args.eps,
    ...
    _, z_auto_b = autoattack_reference_points(
        model=model_b,
        x=x_sub,
        y=y_sub,
        eps=args.eps,
